...
SelectorGadget.prototype.setupBorders = function() {
  if (!this.b_top) {
    ...
    //this.b_bottom = jQuery('<div>').addClass('sg_border').addClass('sg_bottom_border').css('height', this.px(this.border_width + 20)).hide().bind("mousedown.sg", { 'self': this }, this.sgMousedown);
    this.b_bottom = jQuery('<div>').addClass('sg_border').addClass('sg_bottom_border').hide().bind("mousedown.sg", { 'self': this }, this.sgMousedown);
...
SelectorGadget.prototype.suggestPredicted = function(prediction) {
  if (prediction && prediction != '') {
    ...
    jQuery(prediction).each(function() {
      ...
      if (!jQuery(this).hasClass('sg_selected') && !jQuery(this).hasClass('sg_ignore'));
    });
